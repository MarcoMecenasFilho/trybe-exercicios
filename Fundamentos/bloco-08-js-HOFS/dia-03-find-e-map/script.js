...
// Construa um array de objetos a partir do array de livros. Cada objeto 
// deve conter uma propriedade author , com o nome da pessoa autora do livro,
...
//  Crie um array com todos os objetos que possuem gênero ficção
...
function fantasyOrScienceFictionAuthors() {
  const generes = ['Fantasia', 'Ficção Científica'];
  return books.filter((book) => generes.includes(book.genre)).map((book) => book.author.name).sort();
}
...
//  nome registrado começa com três iniciais.

const expectedResult6 = 'O Senhor dos Anéis';

function authorWith3DotsOnName() {
  return books.find((book) =>( 
    book.author.name.split(' ').filter((world) => world.endsWith('.')).length === 3
    )).name;
}
