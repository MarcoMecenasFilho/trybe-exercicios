...
test('testar a chamada da função', () => {
  math.subtrair = jest.fn();
  math.subtrair();
  expect(math.subtrair).toHaveBeenCalled();
});

// 2 - Faça o mock da função multiplicar e implemente como retorno
//     padrão o valor '10'. Teste a chamada e o retorno.

test('multiplcar com retorno padrão 10', () => {
...
// 3 - Faça o mock da função somar e implemente uma função que
//     recebe dois valores e retorna sua soma. Teste a chamada,
//     o retorno e os parâmetros passados.

test('#somar', async () => {
  const mockSomar = jest.spyOn(math, 'somar');
  mockSomar.mockResolvedValue(7);
 
  mockSomar(3, 4);
  expect(mockSomar).toHaveBeenCalled();
  expect(mockSomar).toHaveBeenCalledTimes(1);
  expect(mockSomar).toHaveBeenCalledWith(3, 4);
  await expect(mockSomar(3, 4)).resolves.toBe(7);
});

// 4 - Faça o mock da função dividir e implemente um retorno
//     padrão com o valor '15'. Implemente também os seguintes
//     valores para a primeira e segunda chamadas: '2' e '5'. 
//     Teste a chamada, o retorno, os parâmetros e quantas vezes
//     a função foi chamada.

test('Testa chamada, retorno e parâmetros da funcao dividir', () => {
...
// 5 - Faça o mock da função subtrair de maneira que seja possível
//     restaurar sua implementação original. Defina como retorno 
//     padrão o valor '20'. Teste o número de chamadas e o retorno.
//     Restaure a implementação original da função e teste sua execução.

test('Mock da função subtrair de modo q possa restaurar sua implementação original', () => {
  ...
  expect(mockSubtrair).toHaveBeenCalledTimes(2);// aqui são duas pois no começo do codigo já havia chamado a func();
